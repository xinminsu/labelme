...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        copy = action(self.tr('Duplicate Polygons'), self.copySelectedShape,
                      ...
                      enabled=False)
        saveshape = action(self.tr('Save Polygons'), self.saveSelectedShape,
                      shortcuts['save_polygon'], 'saveshape',
                      self.tr('Save the copied polygons'),
                      enabled=False)
        loadshape = action(self.tr('Load Polygons'), self.loadSavedShape,
                           shortcuts['save_polygon'], 'loadshape',
                           self.tr('Load the saved  polygons'),
                           enabled=False)
        clearshape = action(self.tr('Clear Polygons'), self.clearSavedShape,
                           shortcuts['clear_polygon'], 'clearshape',
                           self.tr('clear the saved  polygons'),
                           enabled=False)
        ...
        self.actions = utils.struct(
            ...
            delete=delete, edit=edit, copy=copy,saveshape=saveshape,loadshape=loadshape,clearshape=clearshape,
    ...
    def shapeSelectionChanged(self, selected_shapes):
        ...
        self.actions.saveshape.setEnabled(n_selected)
    ...
    def saveSelectedShape(self):
        added_shapes = self.canvas.selectedShapes
        for shape in added_shapes:
            if shape not in self.saveShapes:
                self.saveShapes.append(shape)
        if len(self.saveShapes) > 0:
            self.actions.loadshape.setEnabled(True)

    def loadSavedShape(self):
        self.loadShapes(self.saveShapes, replace=False)
        self.setDirty()

    def clearSavedShape(self):
        self.saveShapes.clear()
        self.actions.loadshape.setEnabled(False)
        if len(self.canvas.selectedShapes) == 0:
            self.actions.saveshape.setEnabled(False)
    ...
    def openPrevImg(self, _value=False):
        ...
        if len(self.saveShapes) >0:
            self.actions.loadshape.setEnabled(True)
            self.actions.clearshape.setEnabled(True)
